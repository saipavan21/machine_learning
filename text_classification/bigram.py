...
#data import
train_data = pd.read_csv('reviews_tr.csv').head(500000)
...
count_vec_test = CountVectorizer(ngram_range =(2,2))
...
#training the model
starttime = time.time()
bigram_W = get_weights(X_train, Y_train)
...
train_feat_df['weight'] = bigram_W.T
train_feat_df.to_csv('bigram_W.csv', sep=',',index=False)

#Training Accuracy
train_acc = get_accuracy(X_train, Y_train, bigram_W)
print("Bigram training Accuracy:",train_acc)
...
bigram_test_W = pd.merge(test_feat_df, train_feat_df, on= 'feat', how='left').fillna(0)['weight']

test_acc = get_accuracy(X_test, Y_test, bigram_test_W)
print("Bigram test accuracy:",test_acc)